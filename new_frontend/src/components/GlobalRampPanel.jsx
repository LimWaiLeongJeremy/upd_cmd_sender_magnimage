/**
 * components/GlobalRampPanel.jsx
 * --------------------------------
 * Control panel for running a brightness ramp across selected groups.
 *
 * PROPS:
 *   selectedGroups — string[] of selected group IDs
 *   rampInFlight   — React ref, true while a ramp is running
 *   onRun          — (params) => void
 *   onSelectAll    — () => void
 *   onClearAll     — () => void
 */
...
export default function GlobalRampPanel({
  selectedGroups,
  rampInFlight,
  onRun,
  onSelectAll,
  ...
  // Estimate duration so the operator knows what they're committing to
      ...
      <div className="selected-groups-row">
        {hasSelection ? (
          ...
          <span className="no-selection-hint">
            Select groups on the cards above to include in this ramp
          </span>
        )}
      </div>
